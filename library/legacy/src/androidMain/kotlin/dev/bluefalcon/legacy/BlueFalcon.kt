...
/**
 * Android implementation of legacy BlueFalcon API using AndroidEngine
 */
actual class BlueFalcon actual constructor(
    ...
    private val engine = AndroidEngine(context.androidContext, coreLogger, autoDiscoverAllServicesAndCharacteristics)
    ...
    actual fun scan(filters: List<ServiceFilter>) {
        scope.launch {
            try {
                ...
                // Map to legacy exceptions if needed
                throw e
            }
        }
    }
    ...
    actual fun refreshGattCache(bluetoothPeripheral: BluetoothPeripheral): Boolean {
        return engine.refreshGattCache(bluetoothPeripheral)
    }
    ...
    actual fun openL2capChannel(bluetoothPeripheral: BluetoothPeripheral, psm: Int) {
        scope.launch {
            try {
                ...
                // Socket will be passed through delegate
