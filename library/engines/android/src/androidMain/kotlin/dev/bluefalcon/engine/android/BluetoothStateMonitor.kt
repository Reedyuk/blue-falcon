...
/**
 * Monitors Bluetooth adapter state changes and notifies registered AndroidEngine instances.
 */
internal object BluetoothStateMonitor {
    
    private val instances = mutableListOf<WeakReference<AndroidEngine>>()
    ...
    fun register(context: Context, instance: AndroidEngine) {
        synchronized(instances) {
            instances.removeAll { it.get() == null }
    ...
    fun unregister(context: Context, instance: AndroidEngine) {
        synchronized(instances) {
